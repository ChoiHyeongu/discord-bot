...
class Client(discord.Client):
    ...
    async def on_message(self, message):
        if message.content.startswith(Constant.PREFIX_CHAR):
            ...
            if message.content == '~애들아 나 할말 있어...':
            ...
            if message.content == '~남웅희':
                await message.channel.send('병신ㅋㅋ')
            
            if message.content == '~김민준':
                await message.channel.send('점순')

            if message.content == '~어쩌라는거지':
